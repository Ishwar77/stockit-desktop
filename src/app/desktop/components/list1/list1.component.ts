...
import jsonData from '../../../../assets/data/PeriodicElement.json';
...
const data = jsonData.filter(elements => elements.place.toLowerCase() === this.filtersite.site.toLowerCase());
...
filtercategory() {
  console.log('JSON DATA ', jsonData);
  const data = jsonData.filter(elements => elements.categories.toLowerCase() === this.filterbycategory.category.toLowerCase());
  console.log('DATA = ', data);
  this.dataSource = new MatTableDataSource<PeriodicElement>(data);
  this.dataSource.paginator = this.paginator;
  }
  ...
  filterbrand() {
    console.log('JSON DATA ', jsonData);
    const data = jsonData.filter(elements => elements.brand.toLowerCase() === this.filterbybrand.brand.toLowerCase());
    console.log('DATA = ', data);
    this.dataSource = new MatTableDataSource<PeriodicElement>(data);
    this.dataSource.paginator = this.paginator;
    }
    ...
    filterstore() {
      console.log('JSON DATA ', jsonData);
      const data = jsonData.filter(elements => elements.store.toLowerCase() === this.filterbystore.toLowerCase());
      console.log('DATA = ', data);
      this.dataSource = new MatTableDataSource<PeriodicElement>(data);
      this.dataSource.paginator = this.paginator;
      }
